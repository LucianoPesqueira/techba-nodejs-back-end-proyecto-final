...
describe("CRUD /api/products", () => {
  ...
  test("POST /api/products debe crear un producto", async () => {
    ...
    expect(response.body.category[0]).toBe("Ejemplo");
  ...
  test("PUT /api/products/:id debe actualizar el producto", async () => {
    ...
    expect(Array.isArray(response.body.product.category)).toBe(true);
    expect(response.body.product.category[0]).toBe("Ejemplo");
  });
